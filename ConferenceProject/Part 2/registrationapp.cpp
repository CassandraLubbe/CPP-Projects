...
{
    // Gets user input by providing the information windows individually, to prevent complications wiht data entry
...
void RegistrationApp::deleteRegistration()
{
    ...
    // Takes uer input for the email to ensure correct registration is being deleted
    ...
    // Initializes instance of registration list
    const QList<Registration*>& registrations = registrationList.getRegistrations();

    // Initialize the two items that were located in the table to variables
    QTableWidgetItem* item = items.first();
    QTableWidgetItem* itemTwo = itemsTwo.first();

    // Set row numbes of items found in variables to variable integer
    int row = item->row();
    int rowTwo = itemTwo->row();

    if (row != rowTwo)  // removes the possibility of deleting the worng registration if the data isn't within the same row
    {
    ...
    // Removes the registration from the registrationList data
...
void RegistrationApp::writeToXMLfile()
{
    ...
    if(!filePath.isEmpty()) // if file has been selected, proceed
    {
        RegistrationListWriter writer(registrationList);    // initialize writer for XML file
        writer.writeToXml(filePath);    // write to given file

        QMessageBox::information(this, "Save to XML", "Registration list saved successfully");  // provide user feedback
    }
}
