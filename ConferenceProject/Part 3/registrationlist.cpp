...
{
    m_AttendeeList.append(r);   // adds registration to registration list
}
...
double RegistrationList::totalFee(const QString t) const
{
    ...
    for (Registration* reg : m_AttendeeList)
    {
        ...
        // This if statement matches the selected type "t" with the classname of it's kind. Then calculates the total according
        // to the registration that matches the metaObject classname
        if (t == "All" ||
...
bool RegistrationList::sameEmailDifferentName(const QString &email, const QString &name) const
{
    for (Registration* reg : m_AttendeeList)
    {
        ...
        if (r_email == email)   // if the email matches one that is already in the registration list
        {
            ...
            {
                return false;   // if name matches, user is not registered
            }
        }
        else
        {
...
{
    QMutableListIterator<Registration*> it(m_AttendeeList); // allows for the list to be modfiied during iterations
    // registrations are loaded into the list and then used to perform actions that modify the list
