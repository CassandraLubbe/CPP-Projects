...
void RegistrationListWriter::writeToXml(const QString &filePath)
{
    ...
    const QList<Registration*>& registration = registrationList.getRegistrations(); // read registration into a readable list

    for (const Registration* reg : registration)    // loop through registrations, reading the data to the file
    {
        QDomElement registrationElement = doc.createElement("registrations");
        registrationElement.setAttribute("type", reg->metaObject()->className());   // read the metaObject classname to the registration type
        ...
        if (const StudentRegistration* studentReg = dynamic_cast<const StudentRegistration*>(reg))  // safely convert pointers to the matching registration
        {
        ...
        if (const GuestRegistration* guestReg = dynamic_cast<const GuestRegistration*>(reg))    // safely convert pointers to the matching registration data
        {
        ...
        root.appendChild(registrationElement); // Addd all the above details to the root, to bring the data to a close in XML file
