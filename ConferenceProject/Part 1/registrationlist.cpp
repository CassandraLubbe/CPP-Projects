...
bool RegistrationList::isRegistered(const QString n) const  // checks whether the person is registered
{
...
{
    double total = 0.00;    // ensure it's initialized to zero

    for (Registration* reg : m_AttendeeList)
    {
        QString className = reg->metaObject()->className(); // store class name in variable type
        ...
        }   // compares the classname to the query selected by the user, then calculates accordingly
    }
...
int RegistrationList::totalRegistrations(const QString a) const
{
    ...
    for (Registration* reg : m_AttendeeList)    // cycles through registrations
    {
...
// checks whether the user being registered has a different name when the email matches a registered person
bool RegistrationList::sameEmailDifferentName(const QString &email, const QString &name) const
{
    for (Registration* reg : m_AttendeeList)
    {
        ...
        {
            if (r_name != name) // the name is then compared before allowing the user to be registered
            {
                return true;    // if it's a different name, user is registered
            }
            else
            {
                return false;   // if the name matches, user is not registered
            }
        }
        else
        {
...
bool RegistrationList::deleteRegistration(const QString &name, const QString &email)
{
    ...
    while (it.hasNext())    // while there is still data in the list, continue to read
    {
        ...
        if (reg->getAttendee()->getName() == name && reg->getAttendee()->getEmail() == email)   // if it matches then delete registration
        {
