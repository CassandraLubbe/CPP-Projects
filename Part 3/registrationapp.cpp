    ...
    : QWidget(parent)
{
    ...
    readButton = new QPushButton("Read From XML File", this);
...
void RegistrationApp::deleteRegistration()  // delete a registration
{
    // takes user input for the registration to be deleted
    QString name = QInputDialog::getText(this, "Delete Registration", "Enter name:");

    // finds all matching items in the table for the user given input
    ...
    // takes user input for the email to ensure correct registration is being deleted
    QString email = QInputDialog::getText(this, "Delete Registration", "Confirm email address of registration to be deleted:");

    // locates all items matching the given email
    QList<QTableWidgetItem*> itemsTwo = tableWidget->findItems(email, Qt::MatchExactly);

    if (itemsTwo.isEmpty())
    {
        QMessageBox::warning(this, "Error", "Email does not match data.");  // if the name and email aren't of the same row, error provided
        return;
    }

    // Initialize instance of registrations
    const QList<Registration*>& registrations = registrationList.getRegistrations();
    ...
    // set the row numbers of the items found to variables
    int row = item->row();
...
void RegistrationApp::writeToXMLfile()
{
    ...
    if(!filePath.isEmpty()) // if filepath isn't empty, then proceed
    {
